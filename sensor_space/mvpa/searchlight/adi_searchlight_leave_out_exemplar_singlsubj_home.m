...
% führt Berechnungen auf privaten Arbeitslaptop zu Hause
% durch und speichert sie auf D-Laufwerk sowie auf E-Festplatte ab

   
for ii = 4:length(subjectdir)
    ...
    if 2 == exist([mainpath subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\realigned_data_per_subject\session.mat'], 'file')
        ...
        save(['D:\Arbeit\adidas\data_analysis\single_subjects\' subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\realigned_data_per_subject\session.mat'])
    else
...
%         if ~exist([mainpath subjectdir(ii).name filesep filename  '_' num2str(comp(pp, 1)) '_' num2str(comp(pp, 2)) '_ms_lda.fig'], 'file')
        ...
        %     perf.config.methods = 'bootstrap_pseudotrials';
            ...
            save([mainpath subjectdir(ii).name filesep filename 'perf_pseudotrials_equalnum_x2_trainfold_only_' num2str(time_interval(1)) '_' num2str(time_interval(2)) '_ms.mat'], 'perf')
            save(['D:\Arbeit\adidas\data_analysis\single_subjects\' subjectdir(ii).name filesep filename 'perf_pseudotrials_equalnum_x2_trainfold_only_' num2str(time_interval(1)) '_' num2str(time_interval(2)) '_ms.mat'], 'perf')
