...
%         if ~exist([mainpath subjectdir(ii).name filesep filename  '_' num2str(comp(pp, 1)) '_' num2str(comp(pp, 2)) '_ms_lda.fig'], 'file')
            ...
            [perf, perf_perm] = mvpa_leave_out_balldesign(session, balldesign, comp(pp,:), neighbours);

            if ~exist([mainpath subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\searchlight\realigned_data\'], 'dir')
        ...
        %     perf.config.methods = 'bootstrap_pseudotrials';
           ...
           perf.mean_accuracy_classifiers = (perf.lda.mean_accuracy'+perf.svm.mean_accuracy+perf.logreg.mean_accuracy)./3;
           ...
           for ss = 1:size(perf.lda,2)
               stat = [];
               stat.time = comp(pp, 1);
               stat.mvpa.perf = perf.mean_accuracy_classifiers;
               stat.accuracy = perf.mean_accuracy_classifiers;
            ...
            %    cfg.xlim         = [0, 0];
               cfg.colorbar     = 'yes';
               cfg.title = 'Searchlight mean all classifiers'; 
               figure; ft_topoplotER(cfg, stat); 
               savefig([mainpath subjectdir(ii).name filesep filename '_' num2str(comp(pp, 1)) '_' num2str(comp(pp, 2)) '_ms_mean_all_classifiers.fig'])
               close 
           end
...
function [perf, perf_perm] = mvpa_leave_out_balldesign(session, balldesign, comp, neighbours)
...
%     perf_perm.(config.balldesign{kk}).auc = auc;
    perf_perm.lda.(balldesign{kk})(ss).accuracy = accuracy_lda;    
...
%     for pp = 1:size(clabel_train_fold_rand,1)
    ...
    perf_perm.svm.(balldesign{kk})(ss).mean_accuracy = accuracy_svm;    
...
%     perf_perm.(config.balldesign{kk}).auc = auc;
    perf_perm.logreg.(balldesign{kk})(ss).mean_accuracy = accuracy_logreg;    
