...
for ii = 2:length(subjectdir)
    ...
    path2Ddrive = 'D:\Arbeit\adidas\data_analysis\single_subjects\';
    if ~exist([mainpath subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\realigned_data_per_subject\searchlight\'], 'dir')
   	...
   	load ([path2Ddrive subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\realigned_data_per_subject\session.mat'])
        
    %% MVPA  
    
    [perf_perm] = mvpa_leave_out_balldesign(session, balldesign, time_interval, neighbours);
...
if exist(mainpath, 'dir')
    save([mainpath subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\realigned_data_per_subject\searchlight\permutation_distribution_0.03_0.15.mat'], 'perf_perm')
%         end
end
 save([path2Ddrive subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\realigned_data_per_subject\permutation_distribution_0.03_0.15.mat'], 'perf_perm')
