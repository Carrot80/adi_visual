function  [grandavg, trl_count]= grandavg_sensorspace(subjectpath, grandavg, delete_balldesigns, path2save, path2inputfile, condition)
...
switch condition
        ...
        for ii = 2:length(subjectpath) % wichtig, adi_04 rausnehmen, da andere Kanalsortierung
            ...
            for kk = 1:length(filename)
                ...
                %% z-transformation:
                 ...
                 cfg=[];
                 cfg.template = template.mean_grad;
                 cfg.inwardshift = 1;
                 cfg.headmodel = vol;
                 ...
                 %% avg per run
                 cfg = [];
                 avg_ra = ft_timelockanalysis(cfg, data_ra);
                 all_runs_subject{length(all_runs_subject)+1} = avg_ra;
                 clear cleanMEG_interp cleanMEG_interp_ra avg_ra
    ...
    %         title('grandavg like')
    %         axis tight
            avg_subjects{length(avg_subjects)+1} = avg_like;
            clear avg_like
        end
        ...
        cfg = [];
        cfg.method  = 'across';
        avg_like = ft_timelockgrandaverage(cfg, avg_subjects{:});
        save ('E:\Arbeit\adidas\data_analysis\visual_stimuli\group_analysis\sensor_space\grand_avg\grandavg_like.mat', 'avg_like');
        figure
        plot(avg_like.time, avg_like.avg)
        title('grandavg like')
        axis tight
        savefig (['E:\Arbeit\adidas\data_analysis\visual_stimuli\group_analysis\sensor_space\grand_avg\grandavg_like.fig' ]);
       ...
       case 'dislike'
        ...
        %         title('grandavg like')
        %         axis tight
                avg_subjects{length(avg_subjects)+1} = avg_dislike;
                clear avg_dislike
            end
        ...
        save ('E:\Arbeit\adidas\data_analysis\visual_stimuli\group_analysis\sensor_space\grand_avg\grandavg_dislike.mat', 'avg_dislike');
        figure
        plot(avg_dislike.time, avg_dislike.avg)
        title('grandavg dislike')
        axis tight
        savefig (['E:\Arbeit\adidas\data_analysis\visual_stimuli\group_analysis\sensor_space\grand_avg\grandavg_dislike.fig' ]);
...
cfg = [];
cfg.method  = 'within';
grandavg_like_group = ft_timelockgrandaverage(cfg, grandavg_like{:});
grandavg_dislike_group = ft_timelockgrandaverage(cfg, grandavg_dislike{:});
...
save (['E:\Arbeit\adidas\data_analysis\visual_stimuli\group_analysis\sensor_space\grand_avg\realigned_data\grandavg_like.mat' ], 'grandavg_like_group');
save (['E:\Arbeit\adidas\data_analysis\visual_stimuli\group_analysis\sensor_space\grand_avg\realigned_data\grandavg_dislike.mat' ], 'grandavg_dislike_group');
save (['E:\Arbeit\adidas\data_analysis\visual_stimuli\group_analysis\sensor_space\grand_avg\realigned_data\grandavg_dontcare.mat' ], 'grandavg_dontcare_group');
...
set(gcf,'color','w');
% box off
legend({'all trials like and dislike'; 'like';  'dislike'})
legend('boxoff') 
    
%%

[rms_like] = rms(grandavg_like_group.avg);
[rms_dislike] = rms(grandavg_dislike_group.avg);
...
cfg.latency   = [.03 .15];
cfg.parameter = 'avg';
GA_like_comp1         = ft_timelockgrandaverage(cfg,avg_like);
GA_dislike_comp1       = ft_timelockgrandaverage(cfg,avg_dislike);
...
stat = ft_timelockstatistics(cfg,grandavg_like{:} ,grandavg_dislike{:});   % don't forget the {:}!
...
stat = ft_timelockstatistics(cfg,grandavg_like{:} ,grandavg_dislike{:}); 
...
grandavg_like_group_trls = ft_timelockgrandaverage(cfg, grandavg_like{:});

cfg = [];
cfg.method  = 'within';
cfg.keepindividual = 'yes';
grandavg_dislike_group_trls = ft_timelockgrandaverage(cfg, grandavg_dislike{:});
...
cfg = [];
avglike_trls = ft_timelockanalysis(cfg, grandavg_like_group_trls);
avgdislike_trls = ft_timelockanalysis(cfg, grandavg_dislike_group_trls);
cfg  = [];
cfg.operation = 'subtract';
cfg.parameter = 'avg';
raweffectlike_vs_dislike = ft_math(cfg, avglike_trls, avgdislike_trls);
...
subj = size(grandavg_like_group_trls.individual,1);
...
cfg.clusterstatistic = 'maxsum'; % test statistic that will be evaluated under the
                               % permutation distribution.
cfg.minnbchan = 2;               % minimum number of neighborhood channels that is
cfg.avgoverchan = 'yes';                               % required for a selected sample to be included
                               % in the clustering algorithm (default=0).
%cfg.neighbours = neighbours;   % see below
cfg.tail = 0;                    % -1, 1 or 0 (default = 0); one-sided or two-sided test
cfg.clustertail = 0;
...
%%
ga_left = ga.avg(ind_left,:);
ga_right = ga.avg(ind_right,:);
LikeminDislike = abs(grandavg_like_group.avg)-abs(grandavg_dislike_group.avg);
...
figure;
plot(raweffectlike_vs_dislike.time, LikeminDislike)
